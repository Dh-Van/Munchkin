...
COLORS = {
    ARM_STATE.SCOOP: "\033[92m",   # Green
    ARM_STATE.TRACK: "\033[94m",   # Blue
    ARM_STATE.RECORD: "\033[91m",  # Red
    ARM_STATE.HOME: "\033[93m",    # Yellow
    ARM_STATE.RETRACT: "\033[95m", # Magenta
    ARM_STATE.IDLE: "\033[90m",    # Grey
...
class Munchkin:
    ...
    def __init__(self):
        self.robot = Robot(self.NUM_DOF, self.ARM_LENGTHS, enable=True)
        self.controller = Controller(self.NUM_DOF, self.ARM_LENGTHS)
        ...
        GPIO.setup(self.SHOULDER_ESTOP, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.MAIN_ESTOP, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        ...
        self.timer = 0
        self.depth_limit = 20
        self.depth_in_limit = 60
        self.depth_stay_limit = 20
        
        self.home_time = 0
        
        self.retract_limit = 15
    ...
    def check_estop(self):
        if not GPIO.input(self.SHOULDER_ESTOP) or GPIO.input(self.MAIN_ESTOP):
            return True
        return False
    ...
    def home(self):
        ...
        if(self.timer < 2):
            self.robot.set_theta_list(HOME_ANGLES, time=2 * 1000)
        
        if(self.timer > int(2.0 / self.DT)):
    ...
    def scoop(self):
        ...
        self.robot.set_theta_list(self.recorded_angles[self.loop_iteration], time=self.DT * 1000 * 2.5)
        # self.robot.set_theta_list(self.recorded_angles[self.loop_iteration])
                    
        self.loop_iteration += 2
    ...
    def mouth_tracking(self):
        ...
        if (not data['mouth_open'] and self.timer < 10 and data['tracking']):
        ...
        if(data['tracking']):
            ...
            self.depth_in = True
            self.feed_timer = self.depth_in_limit - 10
    ...
    def retract(self):
            if(self.timer > self.retract_limit):
                ...
                # self.set_state(ARM_STATE.HOME, time_inp=4)
                self.set_state(ARM_STATE.IDLE, time_inp=4)
    ...
    def idle(self):
        ...
        if(self.timer < 2):
            ...
            continue_loop = input("SANITARY WAIT | PRESS ENTER TO KEEP FEEDING: ")
            self.set_state(ARM_STATE.SCOOP)
    ...
    def set_state(self, new_state, next_state=None, time_inp=0):
        # print(f'SWITCHING STATES: {self.state} -> {new_state}')
        c1 = COLORS.get(self.state, COLORS["RESET"])
        c2 = COLORS.get(new_state, COLORS["RESET"])
        reset = COLORS["RESET"]

        print(f'SWITCHING STATES: {c1}{self.state.name}{reset} -> {c2}{new_state.name}{reset}')
